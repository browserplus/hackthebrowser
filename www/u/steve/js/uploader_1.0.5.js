...
BPTool.Uploader = typeof BPTool.Uploader != "undefined" && BPTool.Uploader ? BPTool.Uploader : function() {
    ...
    var fileTmpl = 
        ...
        '   <div id="{dzid}_prog{index}" style="position:absolute; width:0%; height:20px;"></div>' + 
        ...
        create: function(containingDiv, initParams) {
            ...
            return function() {
                ...
                function startUpload() {
                    ...
                    // upload of *all* files completed
                    if (droppedFiles.length === 0) {
                        doingUpload = false;
                        if (ConfigParams.uploadCompleteCB) {
                            ConfigParams.uploadCompleteCB({success: true, id: userDiv});
                        }
                    ...
                    function uploadComplete(res) {
                        if (res.success) {
                            ...
                            if (ConfigParams.uploadCompleteCB) {
                                ConfigParams.uploadCompleteCB({success: false, id: userDiv});
                            }
                            doingUpload = false;
                            renderCoreUI();
                        }
